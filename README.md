# ThinkCode Learning Analytics Platform — Backend Documentation

Welcome to the ThinkCode backend documentation. This document covers the architecture, the domain concepts, the AI agent ecosystem, and instructions on how to set up and run the FastAPI-based backend. ThinkCode aims to deliver a comprehensive learning and analytics environment designed particularly around **Princeton Algorithms**.

---

## 1. System Overview

ThinkCode is an AI-powered Learning Analytics Platform composed of:
1. **Frontend**: A React/Vite-based User Interface (already established).
2. **Backend**: A robust REST API built with **FastAPI**.
3. **Database**: **PostgreSQL** configured for high-performance reading and analytical grouping, accessed via **SQLAlchemy**.
4. **AI Layer**: An agentic AI workflow managed by **LangGraph** & **Langchain** leveraging OpenAI's `gpt-4o-mini` and `gpt-4o` models.
5. **Observability**: **Langfuse** acts as the observability backbone for all language model interactions, keeping track of latency, token usage, and generation steps.

### High-Level Architecture
- Client (React) requests authentication via JWT tokens to the FastAPI gateway.
- Valid requests are routed to specific modules: Admin (Topics/Problems), Submissions (Receives answers/code), Analytics (Dashboards), Tutor (Socratic Chat).
- Complex submissions fall back on LangGraph-powered AI Agents to assess, hint, or grade the student.
- Containerization through `docker-compose` spins up the application, database, and pgAdmin locally alongside isolated networks.

---

## 2. Core Entities and Database Schema

The database utilizes `uuid-ossp` and `pg_trgm` extensions containing key tables separated by logical domains:

* **Users & Roles:** Contains `users` (`student`, `instructor`, `admin`), `classes`, and `enrollments`. 
* **Course Structure:** `topics`, `lessons`, and `materials` logically structure Princeton's content (e.g., QuickSort, Union-Find).
* **Problems:** `problems`, `problem_hints`, `problem_test_cases`, and `problem_options` store the actual assignments. Types include `coding`, `multiple_choice`, and `open_response`.
* **Activity & submissions:** `submissions` log student efforts timestamped, while `ai_grading_results` save semantic evaluations generated by Language Models.
* **Analytics Layer:** Pre-computed states like `student_topic_mastery` and `knowledge_gaps` power fast dashboard loads. Detailed atomic events are tracked contextually via partitioned `learning_events`.
* **AI Conversations:** `ai_tutor_sessions` and `hint_requests` keep histories of student interactions with the autonomous agents.

---

## 3. The AI Layer (LangGraph Agents)

ThinkCode incorporates multiple targeted AI agents using LangGraph for robust, state-based decision trees.

### 3.1 Grading Agent (`app/ai/grading.py`)
- **Trigger:** When a student submits an `open_response` answer via `/api/v1/submissions`.
- **Logic:** Matches the student's semantic response to a known problem `grading_rubric`. Evaluates correctness, calculates a granular score (0.0 to 1.0), and provides constructive feedback alongside reasoning traces. 
- **Model Used:** `gpt-4o-mini`.

### 3.2 Socratic Tutor Agent (`app/ai/tutor.py`)
- **Trigger:** Conversational interactions sent to `/api/v1/tutor/chat`.
- **Logic:** Maintains a stateful chat history containing the context of the current problem. Instructed explicitly to *never directly reveal the final answer*. Instead, it asks guiding questions to direct the student.
- **Model Used:** `gpt-4o-mini`.

### 3.3 Hint Decision Agent (`app/ai/hint.py`)
- **Trigger:** A student explicitly pressing the "Request Hint" button (`/api/v1/submissions/{id}/hint`).
- **Logic:** Uses heuristics based on the number of attempts and previously given hints to escalate through three levels:
    - **Level 1**: Gentle Socratic question.
    - **Level 2**: Partial flaw explanation + follow-up question.
    - **Level 3**: A strong near-answer hint.
  The specific prompt ensures it builds mathematically off predefined textbook hints.

### 3.4 Knowledge Gap Analysis Agent (`app/ai/gap_analysis.py`)
- **Trigger:** Instructor requests a gap scan on the dashboard (`/api/v1/instructor/{class_id}/analyze-gaps`).
- **Logic:** Compiles topics and problems with a failure rate higher than 40%. It parses these into a summarized context and outputs a brief overview of where the class is struggling alongside pedagogical "remedial" classroom actions for the instructor.
- **Model Used:** `gpt-4o`.

### 3.5 Content Summarization Agent (`app/ai/content.py`)
- **Trigger:** Instructor generating lesson reading content (`/api/v1/lessons/{id}/generate-content`).
- **Logic:** Given raw text or a scraped URL (from the Princeton Algorithms site), the agent uses formatting techniques to split content into a brief summary and formatted Markdown readable by students.

---

## 4. Observability and Tracing (Langfuse)

All AI agents are instrumented with **Langfuse**. 
Langfuse tracks every LLM invocation, giving visibility into:
- The exact prompt built by Langchain.
- The actual model parameters (temperature, max tokens).
- Expected token limits vs consumed tokens.
- Cost computation.

Connections to Langfuse are established by standard HTTP callbacks using Langchain's robust integration via environment variables:
`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, and `LANGFUSE_HOST`.

In our pipelines, the trace URLs are occasionally extracted and returned with the API response, saving exactly which trace prompted which database artifact.

---

## 5. Development and Tooling Setup

### 5.1 Local Execution via Docker (Recommended)
You can bring up the full environment, preserving `.env` configurations using Docker Compose:
...
This brings up:
- `thinkcode-db` (Postgres on generic port 5433).
- `thinkcode-pgadmin` (pgAdmin at exposed port 5050).
- `thinkcode-backend` (FastAPI app loaded via Uvicorn).
- `thinkcode-app` (Optional FrontEnd).

### 5.2 Local Execution via Python (Virtual Environment)
For development without Dockerizing the FastAPI layer:
...
```
*(Ensure that `thinkcode-db` from docker-compose is still running for the SQL mappings)*

### 5.3 Database Configuration & Seeding
On your first startup, or after dropping tables, apply migrations using Alembic and populate with generic Princeton curriculum files:
...
# Seed the database (Topics, Problems, 100 Demo Students, 1 Instructor)
PYTHONPATH=. python3 scripts/seed/run_all.py
```

### 5.4 Test Scripts
The repository features two prominent offline scripts to validate API endpoints against AI interactions without mounting a heavy UI:
- `test_ai.py`: Validates Open-Response AI Grading, Tutor Socratic questioning, and Hint delivery mechanism for students.
- `test_phase6.py`: Validates Instructor perspectives by calling Gap Analysis summaries and Lesson Content Generation.

---

## 6. Access Metrics

* **Instructor Credentials:** `instructor@thinkcode.edu` / `Instructor123!`
* **Student Credentials:** `<first.last>@thinkcode.edu` / `Student123!` (e.g., `emma.johnson@thinkcode.edu`)

## 7. Folder Structure
```text
backend/
├── app/
│   ├── ai/                 # LangGraph Agents (grading, hint, tutor, etc.)
│   ├── api/                # API Routers matching standard domains
│   ├── core/               # App configuration (settings, security mappings)
│   ├── db/                 # Alembic models, DB Session mapping
│   ├── analytics/          # Optimized raw SQL queries for reporting
│   └── schemas.py          # Pydantic validation structures
├── scripts/
│   └── seed/               # Curricular structure definitions & fake data setup
├── alembic/                # DB Migration schema definitions
├── requirements.txt        # PIP dependencies
└── main.py                 # FastAPI mounting point
```
