 ...
 * useCourses.ts
 *
 * Backend'den kurs listesini çeker.
 *
 * Instructor → GET /classes/my  → kendi sınıfları
 * Student    → GET /classes/all → tüm aktif kurslar (is_enrolled flag'li)
 *
 * REFRESH-SAFE: token null iken fetch atılmaz.
 * token set olduğunda (login veya localStorage restore) otomatik refetch.
 */
...
// ── API helpers (App.tsx handler'larında kullanılır) ─────────────────────────
 ...
 * @param token     - AuthContext'ten gelen JWT (null = henüz login olmamış)
 *                    Token değiştiğinde (restore/login) otomatik refetch yapılır.
 */
export function useCourses(userRole: UserRole, token: string | null) {
  ...
  const fetchCourses = useCallback(async () => {
    if (!token) return;   // Token yoksa çekme (refresh'te güvenli)

    setIsLoading(true);
    setError(null);

    // Instructor → kendi kursları, Student → tüm aktif kurslar
  ...
  }, [userRole, token]);   // ← token değişince (restore/login) refetch tetiklenir
