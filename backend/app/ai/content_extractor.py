...
Yeni: Smart Deduplication
  - Mevcut sınıf topic'leri GPT'ye gönderilir
  - GPT her yeni topic için "merge_with_topic_id" belirler
  - Aynı konu tekrar yüklenirse duplicate oluşmaz, mevcut topic zenginleşir
"""
...
def extract_content_from_markdown(
    ...
    if len(markdown_text) > MAX_TEXT_CHARS:
        logger.warning(f"Metin çok uzun ({len(markdown_text)} kar), ilk {MAX_TEXT_CHARS} işleniyor.")
        markdown_text = markdown_text[:MAX_TEXT_CHARS]

    # Mevcut class topic'lerini yükle (deduplication için)
    ...
    logger.info(
        f"Extraction tamamlandı: "
        f"{counts['topics_created']} topic (yeni), "
        f"{counts['topics_merged']} topic (güncellendi), "
...
{{
  ...
  "misconceptions": ["Common misconception 1", "Common misconception 2"]
    ...
    logger.info(f"GPT isteği ({len(markdown_text)} karakter, {len(existing_topics)} mevcut topic)...")
    ...
    try:
        ...
        raise RuntimeError(f"GPT geçersiz JSON: {e}\nHam çıktı: {raw[:500]}")
...
def _save_to_database(
    ...
    # id → Topic hızlı erişim
    existing_by_id = {t.id: t for t in existing_topics}

    # Mevcut en yüksek display_order
    ...
    for topic_data in extracted_json.get("topics", []):
        ...
        # ── MERGE: mevcut topic'i zenginleştir ───────────────────────────────
        if merge_id and merge_id in existing_by_id:
            ...
            # Description'ı güncelle (yeni bilgi içeriyorsa)
        ...
        # ── Lessons: başlık eşleşmesi yoksa ekle ─────────────────────────────
        ...
        for i, lesson_data in enumerate(topic_data.get("lessons", [])):
            title = lesson_data.get("title", "Başlıksız Ders")
            if title.lower().strip() in existing_lesson_titles:
                # Aynı başlıklı ders varsa içeriği güncelle (zenginleştir)
                existing_lesson = (
                    db.query(Lesson)
                ...
                logger.info(f"  Lesson GÜNCELLENDI: '{title}'")
            else:
                lesson = Lesson(
                    id=str(uuid.uuid4()),
                    topic_id=topic.id,
                ...
                db.flush()
                lessons_created += 1
                logger.info(f"  Lesson OLUŞTURULDU: '{title}'")

        # ── Problems: başlık duplikatı yoksa ekle ────────────────────────────
        existing_problem_titles = {
            p.title.lower().strip()
            for p in db.query(Problem).filter(Problem.topic_id == topic.id).all()
        }

        # İlk lesson ID'si (problem.lesson_id için)
        first_lesson = (
