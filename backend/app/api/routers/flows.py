...
def generate_flow_summary(
    ...
    """Pedagojik flow'un AI özetini üretir. Sparkles butonuna bağlanır."""
    _check_instructor(current_user)
    flow = _get_flow_or_404(flow_id, db, current_user)
...
# ─────────────────────────────────────────────────────────────────────────────
# SPACED RETRIEVAL — Zamanlanmış tekrar endpoint'leri
# ─────────────────────────────────────────────────────────────────────────────

@router.get("/spaced-reviews")
def get_due_spaced_reviews(
    class_id: str = Query(..., description="Sınıf ID"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
    Öğrencinin bugün (veya geçmişten birikmiş) vadesi gelmiş review'larını döner.
    Dashboard'da 'Bugün Tekrar Zamanı!' kartı için kullanılır.
    """
...
def complete_spaced_review(
    ...
    """
    Öğrenci bir review sorusunu tamamladığında çağrılır.
...
# ─────────────────────────────────────────────────────────────────────────────
# ADAPTIVE BRANCH — Tanı testi endpoint'leri
# ─────────────────────────────────────────────────────────────────────────────
...
def get_adaptive_state(
    ...
    # Tanı için konu başındaki ilk 3 yayınlanmış soruyu seç
    diag_problems = (
        db.query(Problem)
...
def complete_adaptive_diagnostic(
    ...
    Skor hesaplanır, threshold_score ile karşılaştırılır, path atanır.
    """
    # Sınıfın live flow'unu bul
    active_flow = (
        db.query(CourseFlow)
        .filter(CourseFlow.class_id == body.class_id, CourseFlow.status == "live")
        .first()
    )
    threshold = 70  # varsayılan
