...
Hem instructor hem student için tam CRUD + enroll/unenroll endpoint'leri.

GET  /classes/my          → login kullanıcının kursları (enrolled veya owned)
GET  /classes/all         → tüm aktif kurslar (student'ın enroll olanları da işaretli)
POST /classes             → instructor: yeni sınıf oluştur
PUT  /classes/{class_id}  → instructor: sınıf güncelle
DELETE /classes/{class_id}→ instructor: sınıfı pasifleştir (soft delete)
POST /classes/{class_id}/enroll   → student: kayıt ol
DELETE /classes/{class_id}/enroll → student: kaydı sil
"""
...
class ClassCreate(BaseModel):
    ...
    tags:          Optional[str] = ""  # virgülle ayrılmış: "AI,Python,ML"
...
def my_courses(
    ...
    """Kullanıcı rolüne göre kurs listesi — instructor: own, student: enrolled."""
...
def all_courses(
    ...
    """
    Tüm aktif kurslar — her kurs için is_enrolled alanı o öğrenciye göre set edilir.
    Instructor için de kendi kursları döner.
    """
    classes = (
        db.query(Class)
    ...
    # Mevcut kullanıcının enrollment durumu — status dahil (pending de göster)
    if current_user.role in ("instructor", "admin"):
...
def search_classes(
    ...
    """
    Kurs arama: boş query → tüm aktif kurslar, dolu query → tag/name/description ILIKE.
    OpenAI kullanılmaz — frontend filtrelemesini desteklemek için hızlı server-side search.
    """
    ...
    if q and q.strip():
        # Kullanıcının girdiği metni kelimelere böl, her kelime için OR filtresi kur
    ...
    # Mevcut kullanıcının enrollment durumu — status dahil
    if current_user.role in ("instructor", "admin"):
        enrollment_map = {
            cls.id: "active" for cls in
            db.query(Class).filter_by(instructor_id=current_user.id, is_active=True).all()
...
def create_class(
    ...
    """Instructor: yeni sınıf oluştur."""
    if current_user.role not in ("instructor", "admin"):
        raise HTTPException(status_code=403, detail="Only instructors can create classes")

    # Code unique kontrolü — aktif sınıflar arasında unique olmalı
    ...
    # Aynı kodla soft-delete edilmiş varsa sil (unique constraint çakışmasını önle)
...
def delete_class(
    ...
    """Instructor: sınıfı soft-delete (is_active=False) yap."""
    cls = db.get(Class, class_id)
    if not cls or not cls.is_active:
        raise HTTPException(status_code=404, detail="Class not found")
    if cls.instructor_id != current_user.id and current_user.role != "admin":
